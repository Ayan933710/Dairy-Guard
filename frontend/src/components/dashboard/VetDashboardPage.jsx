    ...
    <div className="dashboard-modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="dashboard-modal max-w-lg" role="dialog" aria-modal="true" aria-label="Farm details" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="col-span-2 rounded-lg bg-slate-50 p-3"><span className="text-theme-text-muted">Resolved location</span><strong className="mt-1 block">{location || <LocationLink latitude={farm.hub_latitude} longitude={farm.hub_longitude} />}</strong></div>
        </div>
...
export default function VetDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'recommendations' && <section className="dashboard-panel"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="dashboard-kicker">Priority care queue</p><h2 className="dashboard-section-title">Recommendations</h2><p className="mt-1 text-sm text-theme-text-muted">Contact the farm and open its location directly from each action.</p></div><span className="rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-700">{data.recommendations.length} active</span></div><div className="mt-5 space-y-3">{data.recommendations.length ? data.recommendations.map((item) => { const isAcknowledged = acknowledged.has(item.farm_owner_id); return <article key={item.farm_owner_id} className={`vet-recommendation-card flex flex-wrap items-start justify-between gap-4 rounded-xl border p-4 ${isAcknowledged ? 'is-acknowledged border-emerald-200 bg-emerald-50/60' : 'border-red-200 bg-red-50/60'}`}><div className="min-w-0"><h3 className="font-medium text-theme-text-dark">{item.farm_name || 'Unnamed farm'}</h3><p className="mt-1 text-sm text-theme-text-muted">{item.owner_name}</p><p className="mt-3 flex flex-wrap gap-3 text-xs"><a className="text-sky-700 hover:underline" href={item.owner_phone ? `tel:${item.owner_phone}` : undefined}>Contact: {item.owner_phone || 'Not provided'}</a><LocationLink latitude={item.hub_latitude} longitude={item.hub_longitude} /></p><p className="mt-3 text-sm text-red-800">{item.action} ({item.mastitis_risk_animals} at-risk animals)</p></div><button type="button" className="dashboard-secondary-button" disabled={isAcknowledged} onClick={() => setAcknowledged((current) => new Set(current).add(item.farm_owner_id))}><CheckCircle2 size={14} /> {isAcknowledged ? 'Acknowledged' : 'Acknowledge'}</button></article>; }) : <p className="text-sm text-theme-text-muted">No urgent farm recommendations.</p>}</div></section>}
