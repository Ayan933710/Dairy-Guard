...
const ADMIN_SECTION_ITEMS = [
  { to: '/', label: 'Main Page', icon: Home, end: true },
  { to: '/dashboard/accounts', label: 'Accounts Overview', icon: Users },
  { to: '/dashboard/animals', label: 'Animal', icon: PawPrint },
  { to: '/dashboard/regional-analysis', label: 'Regional Analysis', icon: MapPin },
  { to: '/dashboard/analytics', label: 'Analytics', icon: LineChart },
  { to: '/dashboard/admin-history', label: 'History', icon: History },
  { to: '/dashboard/vet-requests', label: 'Vet Requests', icon: ClipboardCheck },
];
    ...
    <div className="relative flex min-h-screen isolate bg-theme-bg-main text-theme-text-dark">
      ...
      <aside className="z-10 dashboard-sidebar sticky top-0 hidden h-screen w-60 shrink-0 self-start border-r border-slate-200 bg-theme-bg-card px-4 py-6 shadow-sm md:flex md:flex-col">
          ...
          {isVet && isApprovedVet && (
            <div className="ml-2 mt-2 space-y-1 border-l border-slate-300 pl-3">
              {VET_SECTION_ITEMS.map(({ id, label, icon: Icon }) => {
                ...
                return (
                  <NavLink key={id} to={id === 'overview' ? '/dashboard' : `/dashboard?section=${id}`} className={`focus-ring flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition ${active ? 'bg-sky-50 text-theme-primary' : 'text-theme-text-muted hover:bg-slate-50 hover:text-theme-text-dark'}`} onClick={() => {
        ...
        <div className="mt-auto border-t border-slate-200 pt-4">
          <button type="button" onClick={() => { setIsAccountPanelOpen(true); setIsAccountEditing(false); }} className="focus-ring flex w-full items-center justify-between rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2.5 text-left transition hover:border-sky-300 hover:bg-sky-50/80">
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-[10px] uppercase tracking-[0.18em] text-theme-text-muted">My Account</p>
        ...
        <>
          ...
          <aside className="fixed right-0 top-0 z-50 flex h-screen w-[min(460px,92vw)] flex-col border-l border-slate-200 bg-theme-bg-card shadow-2xl shadow-slate-900/10" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              ...
              <button type="button" onClick={() => setIsAccountPanelOpen(false)} className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-sm text-theme-text-dark hover:border-slate-300">Close</button>
            </div>
            ...
            <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
              ...
              <form className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-4" onSubmit={(event) => { event.preventDefault(); setIsAccountEditing(false); setIsAccountPanelOpen(false); }}>
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-theme-text-dark">Edit details</h3>
                  <button type="button" onClick={() => setIsAccountEditing((current) => !current)} className="rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-xs text-theme-text-dark">{isAccountEditing ? 'Cancel edit' : 'Edit'}</button>
                </div>
                ...
                {isAccountEditing ? (
                  <>
                    <label className="block text-xs text-theme-text-muted">
                      ...
                      <input className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-theme-text-dark outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" value={accountForm.full_name} onChange={(event) => setAccountForm((current) => ({ ...current, full_name: event.target.value }))} />
                    </label>
                    <label className="block text-xs text-theme-text-muted">
                      <span className="mb-1 block">Email</span>
                      <input className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-theme-text-dark outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" value={accountForm.email} onChange={(event) => setAccountForm((current) => ({ ...current, email: event.target.value }))} />
                    </label>
                    <label className="block text-xs text-theme-text-muted">
                      <span className="mb-1 block">Phone</span>
                      <input className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-theme-text-dark outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" value={accountForm.phone} onChange={(event) => setAccountForm((current) => ({ ...current, phone: event.target.value }))} />
                    </label>
                    {(user?.role === 'farmer' || user?.role === 'cooperative_admin') && (
                      <label className="block text-xs text-theme-text-muted">
                        <span className="mb-1 block">Farm name</span>
                        <input className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-theme-text-dark outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" value={accountForm.farm_name} onChange={(event) => setAccountForm((current) => ({ ...current, farm_name: event.target.value }))} />
                      </label>
                    )}
                    {user?.role === 'vet' && (
                      <>
                        <label className="block text-xs text-theme-text-muted">
                          ...
                          <input className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-theme-text-dark outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" value={accountForm.vet_designation} onChange={(event) => setAccountForm((current) => ({ ...current, vet_designation: event.target.value }))} />
                        </label>
                        <label className="block text-xs text-theme-text-muted">
                          <span className="mb-1 block">District</span>
                          <input className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-theme-text-dark outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" value={accountForm.vet_district} onChange={(event) => setAccountForm((current) => ({ ...current, vet_district: event.target.value }))} />
                        </label>
                      </>
                    )}
                    <div className="flex gap-2 pt-1">
                      ...
                      <button type="button" onClick={() => {
                        ...
                        setIsAccountEditing(false);
                      }} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-theme-text-dark">Reset</button>
                    </div>
                  </>
                ) : (
      ...
      <div className="relative z-10 min-w-0 flex-1">
        <header className="dashboard-topbar sticky top-0 z-20 flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-theme-bg-card px-4 py-4 shadow-sm sm:px-6 md:px-8">
          <div className="min-w-0">
            ...
            <p className="truncate font-display text-lg text-theme-text-dark">{isAdmin ? 'Administrators Console' : isVet ? 'Veterinary Officer Console' : t('healthConsole')}</p>
          </div>
        ...
        <nav className="dashboard-mobile-nav md:hidden" aria-label="Dashboard navigation">
          {isAdmin ? ADMIN_SECTION_ITEMS.map(({ to, label, icon: Icon, end }) => <NavLink key={to} to={to} end={end} className={({ isActive }) => `dashboard-mobile-link ${isActive ? 'is-active' : ''}`}><Icon size={15} /><span>{label}</span></NavLink>) : isVet ? (isApprovedVet ? VET_SECTION_ITEMS.map(({ id, label, icon: Icon }) => <NavLink key={id} to={id === 'overview' ? '/dashboard' : `/dashboard?section=${id}`} className={({ isActive }) => `dashboard-mobile-link ${isActive ? 'is-active' : ''}`}><Icon size={15} /><span>{label}</span></NavLink>) : null) : visibleNavItems.slice(1).map(({ to, label, icon: Icon, end }) => (
