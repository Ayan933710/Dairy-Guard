...
function ManualAnimalModal({ onClose, onAdd, existingTags }) {
  ...
  async function handleSubmit(event) {
    ...
    const displayTag = form.display_tag.trim().toUpperCase();
    if (existingTags.includes(displayTag.toLowerCase())) {
      setError(t('duplicateId'));
      return;
    }
    if (!/^[A-Za-z0-9]{8,15}$/.test(form.rfid_tag.trim())) {
      setError('RFID / AIN tag must be 8-15 alphanumeric characters.');
      return;
    }
    ...
    <div className="dashboard-modal-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        <form className="dashboard-animal-form" onSubmit={handleSubmit}>
          <label><span>{t('animalType')}</span><select name="species" value={form.species} onChange={updateField}><option value="cow">{t('cowLabel')}</option><option value="buffalo">{t('buffaloLabel')}</option></select></label>
          <label><span>{t('name')}</span><input name="name" value={form.name} onChange={updateField} placeholder="e.g. Ganga" required /></label>
          <label><span>{t('animalId')}</span><input name="display_tag" value={form.display_tag} onChange={updateField} placeholder="e.g. C-142" required /></label>
          <label><span>{t('rfidTagLabel')}</span><input name="rfid_tag" value={form.rfid_tag} onChange={updateField} placeholder="e.g. DG7A91K2" pattern="[A-Za-z0-9]{8,15}" maxLength="15" required /></label>
          ...
          <label><span>{t('lactationNumberLabel')}</span><input name="lactation_number" type="number" min="0" max="15" value={form.lactation_number} onChange={updateField} /></label>
...
export default function HerdOverviewPage() {
  ...
  const highRisk = [...animals]
    .filter((a) => a.risk === 'High Risk' || a.risk === 'Moderate Risk')
    .sort((a, b) => b.riskScore - a.riskScore);
  const avgRisk = animals.length
    ? Math.round(animals.reduce((s, a) => s + a.riskScore, 0) / animals.length)
    : 0;
      ...
      <AnimatePresence>
        {isModalOpen && (
          <ManualAnimalModal
            ...
            onAdd={addAnimal}
            existingTags={animals.map((animal) => animal.displayTag.toLowerCase())}
          />
        )}
      </AnimatePresence>
    </div>
